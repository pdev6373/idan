...
export const ListingPreview = () => {
  ...
  const getDetails = async () => {
    try {
      const res = await axios.post(
        `${store.url}/explorer/explore/accomodation_listings/${id}`,
        {},
        {
          headers: { Authorization: `Bearer ${store.auth.token}` },
        }
      );
      setItem(res?.data?.body?.listing_details);
    } catch (error) {
      // console.log(error);
      return error;
    }
  ...
  const getListerDetails = async () => {
    try {
      const res = await axios.post(
        `${store.url}/explorer/explore/accomodation_listings/${id}/agent_details`,
        {},
        {
          headers: { Authorization: `Bearer ${store.auth.token}` },
        }
      ...
      console.log("res agent", res?.data?.body);
      setLister(res?.data?.body?.agent_details);
    } catch (error) {
      // console.log(error);
      return error;
    }
  ...
  return (
    <div>
      ...
      <section className="px-32 py-6 mt-6">
        <div className="bg-white rounded-[32px] flex p-6">
          <img
            src={item?.accom_images && item?.accom_images[0]}
            className="w-full h-[41rem] rounded-[24px] object-cover object-center"
            alt=""
          />
        </div>
      </section>
      <section className="px-32 py-6 mb-6">
        <div className="bg-white flex flex-col p-12 rounded-[24px]">
          <div className="flex justify-between items-center w-full">
            <div className="flex flex-col">
              ...
              <p className="text-gray-500">
                {item?.accomodation_address}, {item?.accomodation_city},{" "}
                {item?.accomodation_state}
              </p>
              <p className="text-gray-500 flex items-center gap-3 mt-2">
                <p>{`${item?.number_of_rooms} Bedroom`}</p>
                <p>.</p>
                <p>{`${item?.number_of_bathrooms} Bathroom`}</p>
                <p>.</p>
                <p>{`${item?.number_of_kitchens} Kitchen`}</p>
              </p>
            </div>
                  ...
                  <div className="flex gap-56">
                    <div>
                      <h2 className="text-[#878788] ">
                        {listing === "service" ? "Artisan" : "Company"} address
                      </h2>
                      <p>
                        {lister?.brand_address}, {lister?.city},{" "}
                        {`${lister?.state
                          ?.slice(0, 1)
                          ?.toUpperCase()}${lister?.state
                          ?.slice(1)
                          ?.toLowerCase()}`}{" "}
                        state
                      </p>
                    ...
                    <div>
                      ...
                      <p>{lister?.brand_name}</p>
                    </div>
                  </div>
                  <div className="flex gap-56">
                    <div>
                      <h2 className="text-[#878788]">Phone Number</h2>
                      <p>{lister?.phone_number}</p>
                    </div>
                    {/* <div className="flex gap-4 items-center">
                      <svg
                        width="26"
